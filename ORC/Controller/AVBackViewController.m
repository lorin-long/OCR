...
@interface AVBackViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
@implementation AVBackViewController
...
- (void)viewDidLoad {
    ...
    IDBackView *IDCardScaningView = [[IDBackView alloc] initWithFrame:self.view.frame];
    ...
    if (CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess) {
        ...
        if (ret <= 0) {
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                IDBackViewController*IDInfoVC = [[IDBackViewController alloc] init];
